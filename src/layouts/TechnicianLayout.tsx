...
export const TechnicianLayout: React.FC = () => {
  ...
  const navigation = [
    ...
    { name: 'Jobs', href: '/technician/jobs', icon: JobsIcon },
    { name: 'Clock In/Out', href: '/technician/clock-in-out', icon: ClockIcon },
    { name: 'Schedule', href: '/technician/schedule', icon: ScheduleIcon },
    { name: 'Job Log', href: '/technician/job-log', icon: JobLogIcon },
    { name: 'Resources', href: '/technician/library', icon: LibraryIcon },
    { name: 'Inventory', href: '/technician/inventory', icon: InventoryIcon },
    { name: 'AI Assistant', href: '/technician/ai-assistant', icon: AIAssistantIcon },
    { name: 'Settings', href: '/technician/settings', icon: SettingsIcon },
  ];

  const bottomNavigation = [
    { name: 'Help', href: '/technician/help', icon: HelpIcon },
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] dark:bg-gray-900 flex">
      ...
      <div className={`fixed left-0 top-16 bottom-0 bg-white dark:bg-gray-900 shadow-lg z-50 w-64 transform transition-transform duration-300 ease-in-out md:hidden flex flex-col ${
        ...
        <div className="px-3 py-4 space-y-1 border-t border-gray-100 flex-shrink-0">
          {bottomNavigation.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.href}
                onClick={toggleMobileMenu}
                className={`flex items-center gap-3 py-3 rounded-xl transition-all duration-200 hover:bg-gray-50/80 ${
                  sidebarCollapsed ? 'px-4' : 'px-3'
                }`}
              >
      ...
      <div className={`hidden md:flex fixed left-0 top-20 bottom-0 bg-white dark:bg-gray-900 shadow-lg flex-col transition-all duration-300 ease-in-out ${
            ...
            return (
              ...
              >
                ...
                {!sidebarCollapsed && (
                  <span 
                    className={`font-medium transition-colors duration-200 ${
                      active ? 'text-[#10BF0A]' : 'text-gray-700 dark:text-gray-300 dark:text-gray-300 group-hover:text-gray-900'
                    }`}
                  >
                    {item.name}
                  </span>
        ...
        {/* Bottom Navigation */}
        <div className="px-4 py-4 space-y-2 border-t border-gray-100 dark:border-gray-700">
          {bottomNavigation.map((item) => {
            const Icon = item.icon;
            ...
            return (
              <Link
                key={item.name}
                to={item.href}
                className={`flex items-center gap-3 py-3 rounded-xl transition-all duration-200 hover:bg-gray-50/80 group relative ${
                  sidebarCollapsed ? 'px-4' : 'px-3'
                }`}
              >
                ...
                {!sidebarCollapsed && (
                  <span className="font-medium text-gray-700 dark:text-gray-300 dark:text-gray-300 group-hover:text-gray-900 transition-colors duration-200">
                    {item.name}
                  </span>
                )}
                {/* Tooltip for collapsed state */}
      ...
      <button
        ...
        className={`hidden md:block fixed top-24 z-40 p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 dark:border-gray-700 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 group ${
      ...
      >
        <div className="relative">
          {sidebarCollapsed ? (
            <ChevronRight className="w-4 h-4 text-gray-600 group-hover:text-gray-800 dark:hover:text-gray-200 transition-colors" />
          ) : (
            <ChevronLeft className="w-4 h-4 text-gray-600 group-hover:text-gray-800 dark:hover:text-gray-200 transition-colors" />
          )}
        </div>
      </button>
