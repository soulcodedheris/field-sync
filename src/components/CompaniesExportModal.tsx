...
export interface CompaniesExportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onExport: (config: any) => void;
}

export const CompaniesExportModal: React.FC<CompaniesExportModalProps> = ({ isOpen, onClose, onExport }) => {
  const [exportConfig, setExportConfig] = useState({
    format: 'csv',
    dateRange: 'all',
    includeColumns: ['name', 'email', 'status', 'plan', 'users', 'jobsPosted'],
    filters: {
      status: 'all',
      plan: 'all'
    }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Export Companies Data</h2>
          </div>
          <button
            onClick={onClose}
